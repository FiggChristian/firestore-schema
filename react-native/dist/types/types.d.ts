import { ConvertDocumentSchemaType, Expand, GenericDocumentSchema, GenericFirestoreCollection, GenericFirestoreDocument, SchemaOfDocument, StrKeyof, UnionOfTuplesToIntersection } from "@firestore-schema/core";
 ...
 */
export type GettableFirestoreDataTypeNoArray = string | number | boolean | null | FirebaseFirestoreTypes.Timestamp | FirebaseFirestoreTypes.Blob | FirebaseFirestoreTypes.GeoPoint | FirebaseFirestoreTypes.DocumentReference | {
 ...
 * Arrays since Arrays are only allowed to be one-dimensional.
 */
export type SettableFirestoreDataTypeNoArray = GettableFirestoreDataType | Date | {
    [key: string]: SettableFirestoreDataType;
};
/**
 * The value types that can be uploaded to Firestore in a document's fields.
 */
export type SettableFirestoreDataType = ValueOrArray<SettableFirestoreDataTypeNoArray>;
/**
 ...
 */
export type GettableDocumentSchema<Document extends GenericFirestoreDocument> = Expand<ConvertDocumentSchemaType<SchemaOfDocument<Document>, Date, FirebaseFirestoreTypes.Timestamp>>;
/**
 ...
 */
export type SettableDocumentSchema<Document extends GenericFirestoreDocument> = Expand<ConvertDocumentSchemaType<SchemaOfDocument<Document>, Date | FirebaseFirestoreTypes.Timestamp, Date | FirebaseFirestoreTypes.Timestamp>>;
/**
 ...
 */
export type EnsureDocumentHasKey<Collection extends GenericFirestoreCollection, Key extends string> = Collection extends GenericFirestoreCollection ? Expand<Pick<Collection, {
 ...
 * This type is used by `QueryWrapper.where`.
 */
export type EnsureDocumentKeyDoesNotExtendValue<Collection extends GenericFirestoreCollection, Key extends string, Extends> = Collection extends GenericFirestoreCollection ? Expand<Pick<Collection, {
 ...
 * This type is used by `QueryWrapper.where`.
 */
export type EnsureValueExtendsDocumentKey<Collection extends GenericFirestoreCollection, Key extends string, Extends> = Collection extends GenericFirestoreCollection ? Expand<Pick<Collection, {
...
export type WithFieldValue<T extends object> = T extends object ? {
    [K in keyof T]: (T[K] extends infer R ? R extends object ? WithFieldValue<R> : R : never) | FirebaseFirestoreTypes.FieldValue;
} : never;
/**
...
export type DottedFieldValues<T extends object> = T extends object ? {
    [K in StrKeyof<T>]: K | (T[K] extends infer R ? R extends object ? `${K}.${DottedFieldValues<R>}` : never : never);
}[StrKeyof<T>] : never;
/**
 ...
 * ```
 */
export type DottedFieldNameIndex<Field extends string, T extends object> = UnionOfTuplesToIntersection<NestedDottedFieldNameIndex<Field, T>>;
type NestedDottedFieldNameIndex<Field extends string, T extends object> = T extends object ? Field extends `${infer Pre}.${infer Post}` ? Pre extends StrKeyof<T> ? T[Pre] extends object ? NestedDottedFieldNameIndex<Post, T[Pre]> : never : never : T extends object ? Field extends StrKeyof<T> ? [T[Field]] : never : never : never;
/**
 ...
 */
export type SetData<T extends GenericDocumentSchema> = T extends GenericDocumentSchema ? Expand<WithFieldValue<T>> : never;
/**
 * Given a document schema, returns the type of data that can be used to update
 * the data (i.e., what can be passed to `.doc().update()`).
 */
export type UpdateData<T extends GenericDocumentSchema> = T extends GenericDocumentSchema ? Expand<Partial<WithFieldValue<WithDottedFieldNames<T>>>> : never;
/**
 ...
 */
export type DotNestedSchemaKeysOf<Document extends GenericFirestoreDocument> = Document extends GenericFirestoreDocument ? SchemaOfDocument<Document> extends infer R ? R extends object ? string extends StrKeyof<R> ? never : DottedFieldValues<R> : never : never : never;
export {};
