...
class FirestoreWrapper<FirestoreSchema extends GenericFirestoreSchema>
  // Omit<..., never> gets rid of `private` members that cannot be implemented
  // here.
  implements Omit<FirebaseFirestoreTypes.Module, never>
{
  ...
  castToSchema<Path extends string>(
    value: FirebaseFirestoreTypes.DocumentSnapshot,
   ...
   */
  castToSchema<Path extends string>(
    value: DocumentWrapper<GenericFirestoreDocument>,
    optionalPath?: Path | undefined
  ): DocumentWrapper<SchemaAtPath<FirestoreSchema, Path, true>>;
  /**
   * Casts an object to the specified schema as determined by the path to the
   * document/collection.
   *
   ...
   */
  castToSchema<Path extends string>(
    value: FirebaseFirestoreTypes.DocumentReference,
    optionalPath?: Path | undefined
   ...
   */
  castToSchema<Path extends string>(
    value: CollectionWrapper<GenericFirestoreCollection>,
    optionalPath?: Path | undefined
  ): CollectionWrapper<SchemaAtPath<FirestoreSchema, Path, true>>;
  /**
   * Casts an object to the specified schema as determined by the path to the
   * document/collection.
   *
   ...
   */
  castToSchema<Path extends string>(
    value: FirebaseFirestoreTypes.CollectionReference,
    optionalPath?: Path | undefined
   ...
   */
  castToSchema<Path extends string>(
    value: QueryWrapper<GenericFirestoreCollection>,
    optionalPath?: Path | undefined
  ): QueryWrapper<SchemaAtPath<FirestoreSchema, Path, true>>;
  /**
   * Casts an object to the specified schema as determined by the path to the
   * document/collection.
   *
   ...
   */
  castToSchema<Path extends string>(
    value: FirebaseFirestoreTypes.Query,
    optionalPath?: Path | undefined
  ): FirebaseFirestoreTypes.Query<SchemaAtPath<FirestoreSchema, Path, true>>;
  /**
   * Casts an object to the specified schema as determined by the path to the
   * document/collection.
   *
  ...
  collection(
    ...
    if (additionalSegments.length > 0) {
      ...
      let lastDocRef: FirebaseFirestoreTypes.DocumentReference;
   ...
   * ```
   */
  runTransaction<ReturnType>(
    updateFunction: (transaction: TransactionWrapper) => Promise<ReturnType>
  ): Promise<ReturnType> {
    return this.firestore.runTransaction((transaction) =>
      updateFunction(new TransactionWrapper(transaction))
    );
  }
  ...
  loadBundle(
    bundle: string
  ): Promise<FirebaseFirestoreTypes.LoadBundleTaskProgress> {
   ...
   * ```
   */
  namedQuery<Collection extends GenericFirestoreCollection>(
    name: string
  ...
  // `Module.namedQuery` method because the return type is not assignable to the
  // base `Module.namedQuery`'s return type. This is due to the way
  // `QueryWrapper`s work based off collections as their generic type but the
  // base `Query` type works based off a single document. Adding this overload
  // fixes that.
  namedQuery<Collection extends GenericFirestoreCollection>(
    name: string
  ): QueryWrapper<Collection>;
  namedQuery<Collection extends GenericFirestoreCollection>(
    ...
    return new QueryWrapper<Collection>(this.firestore.namedQuery(name));
  }
